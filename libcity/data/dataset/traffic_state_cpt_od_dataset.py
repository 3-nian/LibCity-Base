...
class TrafficStateCPTODDataset(TrafficStateDataset):
    """
    交通状态预测数据集的另一个基类
    ...
    **数据原始的时间戳不能为空！**。
    ...
    def _generate_input_data(self, df):
        ...
        max_t = abs(df.shape[0] - abs(max(y_offsets)))
        # 从最大的偏移处开始算，因为要向前偏移这么多
        x, y, ts_x, ts_y = [], [], [], []
        ts_dumped = []  # 错误时间戳
        for cur_ts in range(largest_interval, max_t):#self.timesolts[largest_interval:]:
            # 求当前时间片cur_ts向左偏移offset_mat之后得到的时间片
            # ts_mat和offset_mat形状相同([[closeness], [period], [trend]])，存储的是具体的时间戳
            # ts_mat:[[671,670,669,668],[624,576],[336,0]]
            ts_mat = \
                [
                    [
            ...
            dat_mat = [[df[ts] for ts in ts_seq] for ts_seq in ts_mat]
