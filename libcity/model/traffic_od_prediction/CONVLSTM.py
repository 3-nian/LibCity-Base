...
class CONVLSTM(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.len_closeness = self.data_feature.get('len_closeness', 12)
        ...
        # self.GCC = GCC()
        self.ConvLSTM = ConvLSTM(
            input_dim=self.c_lt * 2, hidden_dim=128, kernel_size=(3, 3), num_layers=self.n_layers, batch_first=True)
    ...
    def forward(self, batch):
        ...
        # x : (B, T, H, W, H, W)
        xt = x.permute((0, 1, 4, 5, 2, 3))
        x = x.reshape((self.batch_size, self.input_window, -1, self.height, self.width))
        # x : (B, T, H * W, H, W)
        xt = xt.reshape((self.batch_size, self.input_window, -1, self.height, self.width))
    ...
    def predict(self, batch):
        ...
        for i in range(self.output_window):
            ...
            y_ = self.forward(batch)  # (batch_size, 1, len_row, len_column, output_dim)
